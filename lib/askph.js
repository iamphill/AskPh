  ...
  request(url, (function (error, response, body) {
    ...
    json['tracks'].forEach(function (track) {
      ...
      var data = {
        ...
        'itunes:keywords': track['tag_list'].replace(' ', ','),
        'itunes:explicit': 'no',
        enclosure: {
          '@': {
            // url: track['download_url'].replace("https", "http") + '?client_id=' + settings['client_id'],
            url: 'http://feeds.soundcloud.com/stream/46582256-the-bugle-the-bugle-194a.mp3',
            // length: track['original_content_size'],
            length: '26049652',
            type: 'audio/mpeg'
          }
        },
        guid: track['download_url'].replace("https", "http") + '?client_id=' + settings['client_id'],
        'itunes:image': {
          '@': {
    ...
    rss: {
      ...
      '#': {
        channel: {
          ...
          'itunes:explicit': 'no',
          link: 'https://www.ph-creative.com',
          'itunes:image': {
            '@': {
